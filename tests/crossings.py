...
class Crossings(Generic[T]):
  ...
  def count_crossings(self):
    ...
    try:
      edges = sorted((index_left[left], index_right[right], weight) for left, right, weight in self.edges)
    except:
      # print(self.edges)
      raise
  ...
  def count_pair_crossings(self):
    ...
    for k, v in weights.items():
      ...
      for i in range(1, self.size_right):
        cumulative_weights_f[k][i] = cumulative_weights_f[k][i - 1] + v.get(i - 1, 0)

      cumulative_weights_b[k] = [0] * self.size_right
      for i in list(range(self.size_right - 1))[::-1]:
        cumulative_weights_b[k][i] = cumulative_weights_b[k][i + 1] + v.get(i + 1, 0)
