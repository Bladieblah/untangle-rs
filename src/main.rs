...
pub fn main() {
  ...
  log::info!("Running benchmark with L = {} R = {} E = {}", nodes_left.len(), nodes_right.len(), edges.len());
  ...
  log::info!("Start: {} edge crossings", timeit("Count crossings", || crossings.count_crossings()));
  timeit("Crossings Benchmark 1e3", || crossings._swap_nodes(1000, 10., &pair_crossings));
  log::info!("1e3: {} edge crossings", crossings.count_crossings());
  timeit("Crossings Benchmark 1e4", || crossings._swap_nodes(10000, 1., &pair_crossings));
  log::info!("1e4: {} edge crossings", crossings.count_crossings());
  timeit("Crossings Benchmark 1e5", || crossings._swap_nodes(100000, 0.1, &pair_crossings));
